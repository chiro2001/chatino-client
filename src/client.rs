...
impl ChatinoClient {
    pub async fn new(sender: UnboundedReceiver<Message>) -> Result<Self> {
        ...
        tokio::spawn(async {
            ...
            let r = client_to_ws.await;
            warn!("client_to_ws finished");
            r.map_err(|e| panic!("{}", e))
        });
        Ok(Self {
            protocol,
...
mod test {
    ...
    fn test_client() -> Result<()> {
        ...
        let runner = async {
            ...
            let ws_to_stdout = {
                let r = reader.for_each(|message| async {
                    let data = message.unwrap().into_data();
                    tokio::io::stdout().write_all(&data).await.unwrap();
                });
                warn!("ws_to_stdout finished");
                r
            };
            pin_mut!(ws_to_stdout, send_async);
            // ws_to_stdout.await;
            future::join(ws_to_stdout, send_async).await;
            warn!("all finished");
        };
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(runner);
